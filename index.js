...
window.onload = function () {
    ...
    this.setTimeout(function () {
        constructUrls();
        newsApiUrls.forEach(u => getNews(u));
    }, 5000);
};
...
function getNews(url) {
    ...
    request.onreadystatechange = function () {
        if (request.readyState === 4 && request.status === 200) {
            ...
            newsRelated = JSON.parse(window.localStorage.getItem("news_latest"));
            if (newsRelated != null && newsRelated.updateTime < getCurrentTimestamp && json.status == "ok") {
                newsRelated.updateTime = getCurrentTimestamp();
                json.articles.slice(0, Math.min(MAX_NEWS_CNT
                    , json.articles.length
                )).forEach(n => newsRelated.newsDetails.push(n));
...
function renderNews(data){
    document.getElementById("card-content-text").innerText= newsRelated.toString();
}
